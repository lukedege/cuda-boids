...
namespace utils::runners
{
	...
	void cpu_ssbo::uniform_grid_calculation(const float delta_time)
	{
		...
		float cube_half_size = sim_params.static_params.cube_size / 2;
		...
		for (size_t i = 0; i < amount; i++)
		{
			...
			accel_blend = sim_params.dynamic_params.alignment_coeff       * ug_bhvr::alignment      (current.boid_id, positions.data(), velocities.data(), boid_cell_indices.data(), cell_idx_range[current.cell_id].start, cell_idx_range[current.cell_id].end, boid_fov)
						+ sim_params.dynamic_params.cohesion_coeff        * ug_bhvr::cohesion       (current.boid_id, positions.data(), boid_cell_indices.data(), cell_idx_range[current.cell_id].start, cell_idx_range[current.cell_id].end, boid_fov)
						+ sim_params.dynamic_params.separation_coeff      * ug_bhvr::separation     (current.boid_id, positions.data(), boid_cell_indices.data(), cell_idx_range[current.cell_id].start, cell_idx_range[current.cell_id].end, boid_fov)
						+ sim_params.dynamic_params.wall_separation_coeff * ug_bhvr::wall_separation(current.boid_id, positions.data(), sim_volume.data());
	...
	{
		// TODO setup functions for repeated code in uniform/coherent grid calculation
		namespace cg_bhvr = behaviours::cpu::coherent_grid;
		// 1) crea l'array per la griglia con una certa gridresolution (number of cells per line) basata sul doppio della boid_fov 
		float boid_fov = sim_params.dynamic_params.boid_fov;
		float cell_size = 2 * boid_fov;
		float grid_resolution = sim_params.static_params.cube_size / cell_size;
		...
		std::vector<cg_bhvr::boid_cell_index> boid_cell_indices(amount);
		// 2) data la loro posizione, affibbia un indice di griglia a ogni boid(indice 3d{ i,j,k } che però può e deve essere linearizzato)
		int x, y, z, linear_index;
		for (size_t i = 0; i < amount; i++)
		{
		...
		// 3) ordina l'array di boid secondo la loro posizione di griglia(scattered=ordina solo il boid index, coherent = ordina pure velocities e positions)
		auto order_by_cell_id = [](const cg_bhvr::boid_cell_index& a, const cg_bhvr::boid_cell_index& b) -> bool { return a.cell_id < b.cell_id; };
		std::sort(boid_cell_indices.begin(), boid_cell_indices.end(), order_by_cell_id);
		...
		std::vector<cg_bhvr::idx_range> cell_idx_range(cell_amount);
		int current_cell = 0, start = 0, read_cell = 0;
		size_t i;
		for (i = 0; i < amount; i++)
		{
		...
		for (size_t i = 0; i < amount; i++)
		{
			accel_blend = sim_params.dynamic_params.alignment_coeff       * cg_bhvr::alignment      (i, positions.data(), velocities.data(), cell_ids.data(), cell_idx_range.data(), boid_fov)
						+ sim_params.dynamic_params.cohesion_coeff        * cg_bhvr::cohesion       (i, positions.data(), cell_ids.data(), cell_idx_range.data(), boid_fov)
						+ sim_params.dynamic_params.separation_coeff      * cg_bhvr::separation     (i, positions.data(), cell_ids.data(), cell_idx_range.data(), boid_fov)
						+ sim_params.dynamic_params.wall_separation_coeff * cg_bhvr::wall_separation(i, positions.data(), sim_volume.data());
