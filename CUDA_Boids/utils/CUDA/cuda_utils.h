...
namespace utils::cuda
{
	namespace checks
	{
		...
		inline void select_device(int device_index = 0)
		{
			...
			checkCudaErrors(cudaGetDeviceProperties(&deviceProp, device_index));
	...
	namespace containers
	{
		...
		{
			float4* arr_dptr;
			checkCudaErrors(cudaMalloc((void**)&arr_dptr, size * sizeof(float4)));
			random_vec4_fill_dptr(arr_dptr, size, min, max);
			checkCudaErrors(cudaMemcpy(arr_hptr, arr_dptr, size * sizeof(float4), cudaMemcpyDeviceToHost));
			checkCudaErrors(cudaFree(arr_dptr));
		}
	}
}